...
avertedbaseline_OMcases_Ext <- casesAverted.sumAcrossYears(OMcases_df %>% select(-setting) %>%
...
avertedbaseline_nSevere_Ext <- casesAverted.sumAcrossYears(nSevere_df %>% select(-setting) %>%
                                                            rename(setting="Extension"),
                                                          cases_col = "nSevere",
                                                          level_aggr = "setting",
                                                          eir_col = "EIR_type",
...
avertedbaseline_expectedDirectDeaths_Ext <- casesAverted.sumAcrossYears(expectedDirectDeaths_df %>% select(-setting) %>%
                                                            rename(setting="Extension"),
...
avertedbaseline_OMcases_Admin1 <- casesAverted.sumAcrossYears(OMcases_df %>%
                                                               select(-setting) %>%
...
avertedbaseline_nSevere_Admin1 <- casesAverted.sumAcrossYears(nSevere_df %>%
                                                               select(-setting) %>%
                                                               mutate(setting=Admin1),
                                                             cases_col = "nSevere",
                                                             level_aggr = "setting",
...
avertedbaseline_expectedDirectDeaths_Admin1 <- casesAverted.sumAcrossYears(expectedDirectDeaths_df %>%
                                                               select(-setting) %>%
                                                               mutate(setting=Admin1),
                                                             cases_col = "expectedDirectDeaths",
                                                             level_aggr = "setting",
...
#cases averted
avertedbaseline_OMcases_Ext %>% filter(Extension=="Demo"&scenario=="Demo")

avertedbaseline_nSevere_Ext %>% filter(Extension=="Demo"&scenario=="Demo")

avertedbaseline_expectedDirectDeaths_Ext %>% filter(Extension=="Demo"&scenario=="Demo")

avertedbaseline_OMcases_Ext %>% filter(Extension=="Geo"&scenario=="Geo")

avertedbaseline_nSevere_Ext %>% filter(Extension=="Geo"&scenario=="Geo")

avertedbaseline_expectedDirectDeaths_Ext %>% filter(Extension=="Geo"&scenario=="Geo")

#eligible population
targeted_pop_Ext %>% select(Extension,target_pop)

#division of cases averted by targeted population
right_join(avertedbaseline_OMcases_Ext,targeted_pop_Ext) %>%
  mutate(across(starts_with("ca"),~./target_pop*1000)) %>%
  filter(Extension=="Demo")

right_join(avertedbaseline_nSevere_Ext,targeted_pop_Ext) %>%
  mutate(across(starts_with("ca"),~./target_pop*1000)) %>%
  filter(Extension=="Demo")

right_join(avertedbaseline_expectedDirectDeaths_Ext,targeted_pop_Ext) %>%
...
right_join(avertedbaseline_OMcases_Admin1 %>% ungroup() %>% select(-age),
           targeted_pop_geo_Admin1) %>%
  mutate(across(starts_with("ca"),~./pop*1000)) %>% filter(Admin1=="Borgou")

right_join(avertedbaseline_nSevere_Admin1 %>% ungroup() %>% select(-age),
           targeted_pop_geo_Admin1) %>%
  mutate(across(starts_with("ca"),~./pop*1000)) %>% filter(Admin1=="Borgou")
